...
## P0.1 — Cadrage temporel « as-of » + traçabilité à toute épreuve *(CRITIQUE)*
...
## P0.2 — Créer un README racine *(CRITIQUE)*
...
## P0.3 — Ajouter une LICENSE *(CRITIQUE)*
...
## P1.1 — Appliquer réellement la méthodologie revendiquée, ou la retirer *(Majeur)*
...
## P1.2 — Réduire la dépendance à une thèse-pivot mono-sourcée *(Majeur)*
...
## P1.3 — Étiqueter fait / interprétation / spéculation *(Majeur)*
...
## P1.4 — Corriger le biais de confirmation / la non-falsifiabilité du scénario central *(Majeur)*
...
## P1.5 — Tracer la provenance au niveau du chiffre *(Majeur)*
...
## P1.6 — Extraire les données dans des fichiers réutilisables *(Majeur)*
...
## P1.7 — Rendre les cartes utilisables hors-ligne *(Majeur → Critique pour un outil OSINT terrain)*
...
## P2.1 — Ré-accentuer le texte français des dashboards *(Majeur visuellement)*
...
## P2.2 — Accessibilité (WCAG) *(Majeur)*
...
## P2.3 — Échelles de barres comparables *(Majeur)*
...
## P2.4 — Réconcilier les incohérences chiffrées résiduelles *(Mineur)*
...
## P2.5 — Meta de partage, favicon, polish HTML *(Mineur)*
...
**Sprint 1 — Débloquer la publication (P0)**
- [x] ~~Décider de l'intention~~ → **tranché : suivi d'événements réels en cours**
- [ ] Note de cadrage « as-of » (date d'état des lieux + caractère évolutif) sur tous les artefacts
- [ ] Corriger les sources mortes (404 IEA/EIA) + archiver les sources clés (Wayback)
- [ ] README racine
- [ ] LICENSE (MIT + CC BY 4.0)
- [ ] `.gitignore` repo-local

**Sprint 2 — Crédibilité OSINT (P1)**
- [ ] Aligner doc↔pratique sur la méthodologie (Admiralty ou retrait)
- [ ] Recouper/requalifier la thèse-pivot Exxon
- [ ] Étiquetage fait/interprétation/spéculation
- [ ] Mini-ACH + indicateur de falsification par scénario
- [ ] Extraction des données en `data/*.csv` + `*.geojson`
- [ ] Provenance source+date au niveau du chiffre
- [ ] Cartes hors-ligne (vendoriser Leaflet + tuiles)

**Sprint 3 — Polish (P2)**
- [ ] Ré-accentuation des 6 dashboards
- [ ] Accessibilité WCAG (contrastes, couleur+texte, `<noscript>`)
- [ ] Échelles de barres comparables
- [ ] Réconciliation des chiffres périmés
- [ ] Meta/OG/favicon, vendorisation polices
- [ ] Hygiène git (commits, branche, CHANGELOG)
